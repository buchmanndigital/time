...
Workflow:
- **Verbundenes E-Mail-Postfach (IMAP, nur Lesen):** Posteingang: list_imap_emails (optional limit, since_days). **Andere Ordner/Unterordner (z. B. unter „Meine Ordner“ in Webmail):** zuerst list_imap_folders, dann den exakten path aus der Antwort für list_imap_emails (mailbox) und für get_imap_email_content **dieselbe mailbox** wie bei der Liste — UIDs gelten nur pro Ordner. Ohne Konto in den Einstellungen die Tool-Meldung nutzen. Inhalte sachlich zusammenfassen, keine technischen IDs in den Text schreiben.
